...
export class UpdateProductUseCase {
  ...
  async execute(id: string, lojaId: string, dto: UpdateProductDto) {
    ...
    const product = await this.products.update(id, lojaId, dto);

    if (product.ativo) {
      await this.syncDispatcher.syncToLinkedDevices(product.id, lojaId);
    }
