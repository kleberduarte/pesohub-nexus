        ...
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              {fields.map((field) => (
                <div key={field.key}>
                  {/* O id vem da chave do campo: este painel é genérico e serve
                      Setor, Fornecedor, Alérgicos e outros — não há rótulo fixo
                      para derivar. */}
                  <label htmlFor={`campo-${String(field.key)}`} className="block text-sm font-medium text-slate-700 mb-1">
                    {field.label}
                    {field.required && <span className="text-red-500"> *</span>}
                  </label>
                  {field.type === "textarea" ? (
                    <textarea
                      id={`campo-${String(field.key)}`}
